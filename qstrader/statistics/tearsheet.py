...
from pandas.core.window import Window, Rolling
...
class TearsheetStatistics(AbstractStatistics):
    ...
    def update(self, timestamp: pd.Timestamp, portfolio_handler: PortfolioHandler):
        """
    ...
    def get_results(self):
        ...
        """
        # Equity
        equity_s: Series = pd.Series(self.equity).sort_index()

        # Returns
        returns_s: Series = equity_s.pct_change().fillna(0.0)

        # Rolling Annualised Sharpe
        rolling: Window | Rolling = returns_s.rolling(window=self.periods)
        ...
        max_dd: ndarray | int | float | complex
        dd_s, max_dd, dd_dur = perf.create_drawdowns(cum_returns_s)

        statistics: dict[str, float | Series | DataFrame] = {}
    ...
    def _plot_equity(self, stats, ax=None, **kwargs):
        ...
        if self.benchmark is not None:
            ...
            benchmark.plot(
                lw=2, color="gray", label=self.benchmark, alpha=0.60, ax=ax, **kwargs
            )
    ...
    def _plot_txt_curve(self, stats, ax=None, **kwargs):
        ...
        if self.benchmark is not None:
            ...
            ax.text(
                9.75,
                4.9,
                "{:.2%}".format(returns_b.std() * np.sqrt(252)),
                fontweight="bold",
                horizontalalignment="right",
                fontsize=8,
            )
    ...
    def _plot_txt_time(self, stats, ax=None, **kwargs):
        ...
        ax.text(
            9.5,
            2.9,
            "{:.2%}".format(yly_max_win_pct),
            fontsize=8,
            fontweight="bold",
            color="red" if yly_max_win_pct < 0 else "green",
            horizontalalignment="right",
        )
        ...
        ax.text(
            9.5,
            1.9,
            "{:.2%}".format(yly_max_loss_pct),
            fontsize=8,
            fontweight="bold",
            color="red" if yly_max_loss_pct < 0 else "green",
            horizontalalignment="right",
        )
